...
if (!user) {
    ...
    function getMoodEmoji(level) {
        if(level===1) return "🌤️ Level 1"; if(level===2) return "🌥️ Level 2"; 
        if(level===3) return "🌧️ Level 3"; if(level===4) return "⛈️ Level 4"; return "🌪️ Level 5";
    }
    ...
    async function loadProfile() {
        try {
            ...
            if (isMyProfile && uData.moodHistory) {
                let moodArray = Object.entries(uData.moodHistory).sort((a,b) => b[0] - a[0]).slice(0, 5);
                if (moodArray.length > 0) {
                    moodHistoryHtml = `<div class="mood-box">`;
                    moodArray.forEach(([ts, level]) => {
                        let date = new Date(parseInt(ts)).toLocaleDateString();
                        moodHistoryHtml += `<span class="mood-badge">${date}: ${getMoodEmoji(level)}</span>`;
                    });
                    moodHistoryHtml += `</div><p style="color:var(--text-muted); font-size:0.75rem; margin-top:5px;">Your recent Mood Logs (Only visible to you)</p>`;
                }
            }
